...
class Workflow
  ...
  def initialize(filename)
    load(filename)
    @riddl_client = Riddl::Client.new("http://localhost:9290/groups")
  end

  def load(filename)
    @workflow = XML::Smart.open(filename)
  end

  def resolve_calls()
    calls = @workflow.find("//flow:call[string(@service-operation)]", {"flow" => "http://rescue.org/ns/controlflow/0.2"})
    calls.each do |call|
      endpoints= Hash.new
      inputs = Hash.new
      outputs = Hash.new
  ...
  def resolve_class_call(call_id, call, endpoints, inputs, outputs, blocks)
    ...
    resource = @workflow.find("//flow:endpoint[@id='#{endpoint}']", {"flow"=>"http://rescue.org/ns/controlflow/0.2"}).first.text.split("/")[0]
    ...
    flows.each do |f|
      f.attributes['id'] = "#{call_id}#{f.attributes['id']}" if f.attributes.include?('id')
  ...
  def resolve_message(message, params)
    ...
    resource = @workflow.find("//flow:endpoint[@id='#{message.parent.attributes['endpoint']}']", {"flow"=>"http://rescue.org/ns/controlflow/0.2"}).first.text.split("/")[0]
  ...
  def do_riddl_call(resource, expectation, params)
    status, resp = @riddl_client.resource(resource).get params
