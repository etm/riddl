function WFGraph (xml, container) {
  ...
  this.generateGraph = function(format) {
    ...
    container.parentNode.setAttribute("height", "1000");
    container.parentNode.setAttribute("width", "480");

    var block = analyze(start, null, 1);
    var width = (block['max_pos']['col']+2)*column_width;
    var height = (block['max_pos']['line']+2)*row_height;
  ...
  var drawSymbol = function (xy, node) {
   // {{{
    var sym_name = node.nodeName;
    var attrs = {'id': node.getAttribute("id"), 'x': xy['col']*column_width-15, 'y':  xy['line']*row_height-30};
